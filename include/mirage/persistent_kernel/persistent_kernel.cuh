...
// Number of XCDs that TAKE PART in a run. 8 is the whole GPU in SPX and
// the default; a smaller value confines fleet to XCDs [0, N) so a run can
// be held inside a single AID without changing the partition mode.
// Must be defined before its first use (the worker early-exit), since an
// undefined name is 0 in #if and would retire every worker.
#ifndef MPK_NUM_XCDS
...
#if MPK_NUM_XCDS < 8
  // Confine the run to XCDs [0, MPK_NUM_XCDS). These blocks never claim
  // a rank, so the per-XCD rank counters stay dense for the XCDs that do
  // take part, and every barrier threshold is sized for the subset.
  if (xcd_id >= MPK_NUM_XCDS) {
    return;
  }
#endif
...
__device__ __forceinline__ void execute_scheduler(RuntimeConfig config,
  ...
  if (threadIdx.x == 0) {
    ...
    if (sched_id < config.num_local_schedulers) {
      ...
      if (config.worker_xcd_map != nullptr) {
        // Only workers on participating XCDs ever register, so waiting for
        // num_workers hangs whenever the run is confined to a subset.
        int _ready_expect = config.num_workers;
#if MPK_NUM_XCDS < 8
        _ready_expect = config.precomp_workers_per_xcd > 0
                            ? config.precomp_workers_per_xcd * MPK_NUM_XCDS
                            : config.num_workers * MPK_NUM_XCDS / 8;
#endif
        while (atomicAdd(config.worker_xcd_ready_count, 0) < _ready_expect) {
          __nanosleep(100);
        }
...
#if MPK_NUM_XCDS < 8
  // Confine the run to XCDs [0, MPK_NUM_XCDS). These blocks take NO part:
  // they claim no scheduler slot, take no worker id and never register in
  // worker_xcd_map, so the participating XCDs see a dense worker set and every
  // threshold is sized for the subset. The whole block leaves together, so the
  // __syncthreads() below are still uniform.
  __shared__ int sub_xcd;
  if (threadIdx.x == 0) {
    sub_xcd = get_current_xcd_id();
  }
  __syncthreads();
  if (sub_xcd >= MPK_NUM_XCDS) {
    return;
  }
#endif
  if (threadIdx.x == 0) {
...
extern "C" void init_persistent_kernel(std::vector<void *> meta_tensors,
...
#ifdef MPK_FUSED_LAYER_BATCHING
  {
    // One task per PARTICIPATING XCD per fused layer, so this is the
    // logical XCD count -- not 8. With a stride of 8 at 4 XCDs the scan
    // pairs up two real layers into each "layer" and finds half of them.
  ...
  fflush(stdout);
  {
    ...
    // Physical divisor: the grid round-robins over all 8 hardware XCDs
    // regardless of how many take part, so the number of blocks that
    // actually land on one active XCD is num_workers/8, not
    // num_workers/NUM_XCDS_PC. Using the logical count here would make
    // the host expect twice the arrivals that exist and hang.
    int workers_per_xcd = num_workers / 8;
    int max_tpw = 512;
