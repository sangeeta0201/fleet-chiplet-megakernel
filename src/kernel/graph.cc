...
void Graph::register_task(char const *task_type, std::vector<int> params) {
  ...
  } else if (name == "argmax_reduce_xrank") {
    int variant_id = task_register->register_argmax_reduce_xrank_task(
        customized->bgraph, params);
  ...
  // Hopper tasks
    ...
    // Un-absorbing kv_b_v adds one more of each: the packed W_UV weight and
