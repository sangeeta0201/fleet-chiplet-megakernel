...
REGIONS = [
    ...
    (31, 32, "router GEMV + sigmoid/bias TopK",
     "1.076  MISLABELLED AND UNATTRIBUTED. The listed verdicts price only the TopK TAIL (k-loop 72%, rank-select +33%, fold 96ns, bias prefetch dead) and those are all NANOSECOND items -- they do not touch the 1.076. paired_span.py: the region is SHARPLY BIMODAL, per-worker span p25 0.80 vs median 12.95 us/layer, because `xcd_rank < oproj_topk_tiles_per_xcd` admits 24/XCD (192) to the block and `t = xcd_rank; t < router_tile_n` admits 16/XCD (128) to the router. 128/232 = 55% OCCUPANCY; 104 workers idle through ~13 us/layer. The slow mode is TIGHT (p25 12.98 -> max 14.81 against an S31 arrival sd of 0.40), the signature of a COMMON RELEASE -- and the o_proj hier_barrier WAIT is deliberately deferred INTO this kernel, so S30->S31 (0.204) is only the ARRIVAL and the wait lives here. The GEMV cannot explain it: 2 dots of K=5120 is ~20 KB/worker, under 1 us even at 41.6 GB/s/CU. NOW SPLIT AND CLOSED (router_poll_vs_work.py, bank SP6, 40 rank-log samples over 5 logs; no new instrument was needed -- the split was already compiled in and nobody had read it). POLL 6.999 us/call (sd 1.099, 54%), WORK 5.877 (sd 0.075 = 1.3%, 46%). cnt[6]/cnt_tail = 128.0 EXACTLY in all 40, confirming the 128/232 from the source gates. NEITHER HALF IS A LEVER: (a) MY OWN 0.44 ms WIDENING CEILING IS RETRACTED -- there are exactly NUM_EXPERTS/EPT = 128 router TILES and each worker already takes one, so the 104 idle workers have nothing to take; EPT=1 makes 256 tiles over 232 workers = a second round, which the source records as measured WORSE. (b) The poll's cross-rank skew is 2.351 us/layer (rank 0 is the MINIMUM poll in 5/5 logs = the last arriver again) = 0.057 ms at the 0.32 transfer coefficient. (c) r0's own 4.942 us poll floor (0.376 ms) is counted time before a barrier -- 3 for 3 negative. (d) The 0.447 ms work half is at MAXIMUM parallelism and its largest part is 0.169 ms: RMSNorm 2.23 / gate dot 2.18 / arrival atomic 1.42 / TopK tail 0.056 us. The instrument contaminates the WORK half upward (each region carries the previous one's atomic; SP6[0] does not), so 0.447 is an UPPER bound and the verdict only strengthens"),
    (32, 5, "routing-ready poll", "0.317"),
    (5, 6, "W13 tiles",
     "1.402  AT 76% OF ITS REAL ROOF -- and note this label reached that premise through a denominator that was WRONG TWICE (74% -> 36% -> 76%). W13 is a 232-WIDE phase (54 tiles/XCD over 29 workers), NOT the 64 the stale `127 live W13 tiles` kernel comment implies, so its roof is 5373 GB/s aggregate and it achieves 53.48 MB / 13.06 us = 4095 (width_corrected_roofline.py, 1052f86). The whole phase holds 0.233 ms of headroom -- which is why MPK_MOE_PF_GROUPS won 28% in a standalone and moved the wall 0.000 three times. Do NOT cite the 74% or the 36%. OPW=16 -1.34ms, split-K +4.12ms, prefetch null. RANK-ASYMMETRIC: rank 0 spends +6.02 us/layer here vs peers 5.80 (peer spread 0.64) -- the shared expert. PRICED AND CLOSED: MPK_SHARED_DUP doubles that excess for +0.106 ms of wall (n=3, inside noise), so a perfect shard is <=0.103 and a 4-way K-shard <=0.077. NO-GO"),
