...
LABEL = {
    ...
    # Re-verified 2026-08-24 against every mpk_stage_stamp() call site. The
    # previous map was shifted by one stamp across the whole attention half:
    # S17 is "qkv_a tiles DONE", not "q_b done", so (16,17) is qkv_a and every
    # attention label after it was reading one phase early. Same shift on the
    # router: S31 is the o_proj->router barrier arrival, S32 is TopK done.
    (2, 16):  "attn-half entry (dispatch)",
    (16, 17): "P1 qkv_a: resid+RMSNorm+[q_a|kv_a] GEMM",
    (17, 18): "P2 qkv_a->q_b BARRIER",
    (18, 19): "P3 q_b: q_a RMSNorm + absorbed q_b + KV append",
    (19, 21): "P4+5 q_b->decode BARRIER + MLA decode",
    (21, 23): "P6+7 decode->merge BARRIER + split-KV merge",
    (23, 24): "P8 attn->o_proj GPU-wide BARRIER",
    (24, 25): "o_proj weight prefetch issue",
    (25, 26): "per-XCD attention release wait",
    (26, 27): "prefetch DMA retire",
    (27, 28): "attn-half exit -> MoE-half entry",
    (28, 29): "MoE entry -> o_proj worker set entered",
    (29, 30): "P9 o_proj GEMV + residual",
    (30, 31): "P10 o_proj->router BARRIER arrival",
    (31, 32): "P11 post-attn RMSNorm + router GEMV + sigmoid/bias TopK",
