...
int main(int argc, char **argv) {
  ...
  // --place bitmask: 1=logits 2=counters 4=out. Homes these in AID0 with
  // the AID-local allocator (MTYPE_RW) instead of spanning hipMalloc
  // (MTYPE_NC). Opt-in and separable: `out` is acquired cross-XCD with a
  // plain sc1 load, and an RW line is not coherent across the boundary, so
  // --place=4 is the one that can hang or go hash-red.
  int place_on = 0;
  char const *tag = "run";
  ...
  if (probe_on) {
    for (int mode = 0; mode < 2; ++mode) {
      ...
      if (attn_b) {
        probe_placement("attn(AID0)", attn, OPROJ_REDUCTION * 2, mode);
        probe_placement("attn(AID1)", attn_b, OPROJ_REDUCTION * 2, mode);
      } else {
        probe_placement("attn_out", attn, OPROJ_REDUCTION * 2, mode);
      }
      if (residual_b) {
        probe_placement("resid(AID0)", residual, OUTPUT_STRIDE * 2, mode);
        probe_placement("resid(AID1)", residual_b, OUTPUT_STRIDE * 2, mode);
      } else if (residual) {
        probe_placement("residual", residual, OUTPUT_STRIDE * 2, mode);
      }
    }
  }
  ...
  if (use_aid) {
    ...
    if (catomic) {
      ...
      HIP_OK(hipMemset(counters, 0, 64 * 1024));
      if (probe_on) {
        printf("[PROBE] --- counters after --catomic=%d ---\n", catomic);
        probe_placement("counters", counters, 64 * 1024, 0);
        probe_placement("counters", counters, 64 * 1024, 1);
      }
    }
    bar = (unsigned int *)(flags + BAR_OFF_INTS);
    bar_b = (unsigned int *)(flags_b + BAR_OFF_INTS);
    // The probe block above runs before these buffers are allocated, so the one
    // line the barrier actually polls has never been measured. `bar` and
    // `bar_b` live inside flags / flags_b, so probe them here. Safe: the chase
    // pattern this writes is zeroed by the hipMemset(flags, ...) below, before
    // any kernel launch.
    if (probe_on) {
      for (int mode = 0; mode < 2; ++mode) {
        printf("[PROBE] --- sync buffers, %s loads ---\n",
               mode ? "device-scope sc0 sc1" : "non-temporal");
        probe_placement("flags(AID0)", flags, sync_bytes, mode);
        probe_placement("flags(AID1)", flags_b, sync_bytes, mode);
      }
    }
  ...
  printf("[%s] srdv=%d place=%d\n", tag, srdv_on, place_on);
